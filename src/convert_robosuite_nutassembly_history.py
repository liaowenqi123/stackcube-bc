...
def add_history(obs: np.ndarray, acts: np.ndarray, history_len: int = 5) -> tuple[np.ndarray, np.ndarray]:
    """
    添加历史观测
    ...
    Returns:
        obs_with_history: (N, obs_dim * history_len) 带历史的观测
        acts: (N, act_dim) 动作（去掉前 history_len-1 个样本）
    """
    N, obs_dim = obs.shape
    ...
    for i in range(N):
        ...
        for j in range(history_len):
            idx = max(0, i - (history_len - 1 - j))
            history.append(obs[idx])
        obs_history.append(np.concatenate(history))
    
    obs_with_history = np.array(obs_history, dtype=np.float32)
    
    return obs_with_history, acts


def main() -> None:
    p = argparse.ArgumentParser(description="Convert robosuite hdf5 demos with history")
    ...
    p.add_argument(
        "--skip-xml-replay",
        action="store_true",
        help="skip loading per-demo model xml / simulator state replay",
    )
    p.add_argument(
        "--history-len",
        type=int,
        default=5,
        help="number of historical observations to include (default: 5)",
    )
    args = p.parse_args()
    ...
    print(f"Original: obs shape={obs.shape}, acts shape={acts.shape}")

    # 添加历史观测
    obs_with_history, acts = add_history(obs, acts, args.history_len)

    print(f"With history: obs shape={obs_with_history.shape}, acts shape={acts.shape}")

    np.savez_compressed(out_path, obs=obs_with_history, acts=acts)
